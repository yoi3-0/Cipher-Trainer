            ...
            <Panel id={id}>
                ...
                <Group style={boxStyle}>
                    {this.state.activeTab === 'methods' && <Div>
                        Здесь расписаны некоторые сопособы решения задач<br/>
                        <h3>Вступление</h3>
                        ...
                        Во-вторых, для более интересного взаимодействия, в случае выхода за границы диапозона 1-32, будут использоваться символы, которые стоят
                        ...
                        <br/>Метод решения такой задачи до смешного тривиальный: требуется применить обратный сдвиг. Например, в случае прибалвения числа к номеру буквы -
                        вычесть то же число из полученного результата (шифра).
                        <h3>Легкий уровень</h3>
                        Задачи этого и последущих уровней более сложные, так как импользуют операцию взятия по модулю. Однако и эту задачу можно решить.<br/>
                        ...
                        Во-первых, рассмотрим сдвиг и число, по модулю которого мы берём. Найдём 60 по модлую 49:
                        это либо 11, либо -38, чтобы найти обратную функцию нужно взять найденные числа с другим знаком.
                        Вычтем из этих чисел по еденице, которая добавлена в конце шифрующей функции, чтобы получить число от 1 до (в данном случае) 49.
                        Сейчас у нас числа -12 и 37.</p><p> Напомним, что в конце нашей функции нам так же следует прибавить еденицу, так как алфавит
                        нумеруется от 1 до 32, эту же еденицу мы должны забрать из уже имеющихся чисел, и так, мы получаем -13(+1) и 36(+1).
                        Теперь осталось просто записать это, использую операцию взятия по тому же модулю. Итак, мы получаем два варианта ответа:
