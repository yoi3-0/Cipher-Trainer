        ...
        return (
            <Panel id={id} className='edu_one'>
                ...
                <Group style={boxStyle}>
                    ...
                    <b>0≤y-1≤b-1</b>, что нас полностью устраивает, учитывая, что мы работаем с остатками. Тогда сделаем замену y_1=y-1 и x_1=x-1, получим следующее уравнение:
                    <b>y_1=(x_1+a+1)mod b</b>.</p><p> Далее мы должны посмотреть на a: <br/>1) если
                        ...
                        a+1>b, то нам следует найти остаток от деления a + 1 на b, а затем его перенести в левую часть уравнения.</p><p>
                                  Уравнение после всех преобразований выше примет один из двух следующих видов: <b>x_1=y_1-a-1</b> или <b>x_1=y_1-(a+1)mod b</b>. Следующим этапом решения будет «навешивание» mod’a на получившееся уравнение. Получим следующие уравнения:
                    <b>x_1=(y_1-a-1)mod b</b> или <b>x_1=(y_1-(a+1)mod b)mod b</b>. Но есть одна загвоздка: y_1-a-1 или y_1-(a+1)mod b могут быть отрицательными, и это помешает нам нормально использовать операцию деления с остатком. Чтобы у нас не возникало проблем, нужно проделать еще одно действие, а именно: добавить к полученным уравнениям b, а затем еще раз провести деление с остатком.
                    </p><p>Полученная формула – обратная к исходной, остается только заменить y_1 на x_1, а x_1 на y_1 расписать их, как x_1=x-1, y_1=y-1 и перенести единицу от y в другую часть уравнения.
                    ...
                    <b>0≤y-1≤b-1</b>, что нас полностью устраивает, учитывая, что мы работаем с остатками. Тогда сделаем замену <b>y_1=y-1</b> и <b>x_1=x-1</b>, получим следующее уравнение:
                    <b>y_1=(x_1∙a+1∙a) mod b</b>. </p> <p>Далее нам предстоит "снять" деление с остатком и перенести 1∙a в левую часть уравнения. Уравнение примет сл. вид: <b>x_1∙a=y_1-1∙a</b>. Чтобы перейти к заключительной части решения, нам необходимо избавиться в левой части уравнения от a, сделать это можно очень просто, домножив обе части уравнения на <b>a^(-1)</b>, где <b>a^(-1)</b>— это обратное a по умножению в кольце вычетов, т.е.
                        <b>a∙a^(-1)=1(mod b)</b> (если провести операцию нахождения остатка от деления над a∙a^(-1), то мы получим 1). Получим следующие уравнения: <b>x_1=(y_1-1∙a) ∙a^(-1)</b>.</p> <p> Осталось только «навесить» mod b и раскрыть x_1 и y_1. Полученная формула станет обратной к исходной.
                        Конечный вид обратной функции:
                    <b>y=(a^(-1)(x-a-1))mod b+1</b> </p>
                    </Div>}
