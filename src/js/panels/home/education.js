            ...
            <Panel id={id} className='edu_one'>
                ...
                <Group style={boxStyle}>
                    {this.state.activeTab === 'methods' && <Div className='edutext'>
                        ...
                        Все уравнения "легкого" типа имеют вид <br/><b>y = (x + a) mod b + 1</b>.<br/> Как же решать подобные односторонние функции? Давайте разбираться.<p>
                        Для начала перенесем единицу из правой части функции в левую, получим уравнение следующего вида: <b>y-1=(x+a)mod b</b>. Заметим, что 1≤x≤b, 1≤ y≤ b, тогда <b>0≤x-1≤b-1</b>,
                        <b>0≤y-1≤b-1</b>, что нас полностью устраивает, учитывая, что мы работаем с остатками. Тогда сделаем замену y<sub>1</sub>=y-1 и x<sub>1</sub>=x-1, получим следующее уравнение:
                        <b>y<sub>1</sub>=(x<sub>1</sub>+a+1)mod b</b>.</p><p> Далее мы должны посмотреть на a: <br/>1) если
                        a+1&#60;b, то мы просто снимаем mod, переносим a+1 на левую сторону и далее работаем с получившимся уравнением; <br/>2) если
                        a+1>b, то нам следует найти остаток от деления a + 1 на b, а затем его перенести в левую часть уравнения.</p><p>
                        Уравнение после всех преобразований выше примет один из двух следующих видов: <br/><b>x<sub>1</sub>=y<sub>1</sub>-a-1</b> или <b>x<sub>1</sub>=y<sub>1</sub>-(a+1)mod b</b>.<br/> Следующим этапом решения будет «навешивание» mod’a на получившееся уравнение. Получим следующие уравнения:
                        <br/><b>x<sub>1</sub>=(y<sub>1</sub>-a-1)mod b</b> или <b>x<sub>1</sub>=(y<sub>1</sub>-(a+1)mod b)mod b</b>.<br/> Но есть одна загвоздка: y<sub>1</sub>-a-1 или y<sub>1</sub>-(a+1)mod b могут быть отрицательными, и это помешает нам нормально использовать операцию деления с остатком. Чтобы у нас не возникало проблем, нужно проделать еще одно действие, а именно: добавить к полученным уравнениям b, а затем еще раз провести деление с остатком.
                    </p><p>Полученная формула – обратная к исходной, остается только заменить y<sub>1</sub> на x<sub>1</sub>, а x<sub>1</sub> на y<sub>1</sub> расписать их, как x<sub>1</sub>=x-1, y<sub>1</sub>=y-1 и перенести единицу от y в другую часть уравнения.
                        Конечный вид обратной функции:<br/>
                        <b>y=(b+(x-a-2)mod b)mod b+1</b> или
                        <b> y=(b+(x-1-(a+1)mod b)mod b)mod b+1</b>.</p>
                        <h3 className='headerEdu'>Средний уровень</h3>
                        Все уравнения "среднего" типа имеют вид <br/><b>y = (x∙a) mod b + 1</b>.<br/> Как же решать подобные односторонние функции? Давайте разбираться.<p>
                        Для начала перенесем единицу из правой части функции в левую, получим уравнение следующего вида: <br/><b>y-1=(x∙a)mod b</b>.<br/> Заметим, что <b>1≤x≤b</b>, <b>1≤y≤b</b>, тогда <b>0≤x-1≤b-1</b>,
                        ...
                        <b>y<sub>1</sub>=(x<sub>1</sub>∙a+1∙a) mod b</b>. Далее нам предстоит "снять" деление с остатком и перенести 1∙a в левую часть уравнения. Уравнение примет сл. вид: <b>x<sub>1</sub>∙a=y<sub>1</sub>-1∙a</b>. Чтобы перейти к заключительной части решения, нам необходимо избавиться в левой части уравнения от a, сделать это можно очень просто, домножив обе части уравнения на <b>a<sup>-1</sup></b>, где <b>a<sup>-1</sup></b>— это обратное a по умножению в кольце вычетов, т.е.
                        <b>a∙a<sup>-1</sup>=1(mod b)</b> (если провести операцию нахождения остатка от деления над <b>a∙a<sup>-1</sup></b>, то мы получим 1). Получим следующие уравнения: <br/><b>x<sub>1</sub>=(y<sub>1</sub>-1∙a)∙a<sup>-1</sup></b>).<br/> Осталось только «навесить» mod b и раскрыть x<sub>1</sub> и y<sub>1</sub>. Полученная формула станет обратной к исходной.
                        Конечный вид обратной функции:
                        <br/><b>y=(a<sup>-1</sup>(x-a-1))mod b+1</b> </p>
                    </Div>}
